...
class XlsxGeneratorService {
    constructor() {
        ...
        this.boldLabels = [
            'Apelido:', 'Código no sistema Domínio:', 'Razão Social:', 'CNPJ:', 
            'Vigência Inicial:', 'Site:', 'Plano Contratado:', 'SLA Para Retorno:',
            'Serviços Contratados', 'Serviço 1:', 'Serviço 2:', 'Serviço 3:', 'Serviço 4:', 'Serviço 5:', 'Serviço 6:',
            'Contato Principal', 'Nome:', 'Cargo:', 'E-mail:', 'Celular:',
            'Entendimento do Negócio', 'Faturamento anual:', 'Regime Tributário:',
            'NF de Entrada:', 'NF de Saída:', 'CT-es de Entrada:', 'CT-es de Saída:',
            'NF de Prestados:', 'NF de Tomados:', 'Funcionários PJ:', 'Centros de Custo:',
    ...
    ensureOutputDir() {
        if (!fs.existsSync(this.outputDir)) {
            ...
            logger.info('📁 Diretório de relatórios criado', { path: this.outputDir });
        }
    }

    // Função para converter valores para números nativos (evita "número armazenado como texto")
    convertToNumber(value, defaultValue = 0) {
        ...
        if (typeof value === 'string') {
            ...
            // Se string vazia após trim, retorna default
            if (trimmed === '') {
                return defaultValue;
            }
            ...
            // Se conversão foi bem sucedida e é um número válido
    ...
    truncateCompanyName(name) {
        ...
        logger.info(`✂️ Nome truncado: "${nameStr}" → "${truncated}" (${nameStr.length} → ${truncated.length} chars)`);
        return truncated;
    }
    ...
    normalizeCompany(raw) {
        ...
        // CORREÇÃO INTELIGENTE DE DADOS DESALINHADOS
        const fixed = this.fixDataMisalignment(c);

        // Keep originals for any other property; return normalized object
        return fixed;
    }

    // Função para corrigir dados desalinhados da API
    fixDataMisalignment(company) {
        const fixed = { ...company };
        
        // CORREÇÃO 1: CONTATOS DESALINHADOS
        ...
        if (!nome.trim() && cargo.trim() && !cargo.includes('@')) {
            logger.info('🔧 Corrigindo desalinhamento de contatos para:', company.nome_fantasia);
            
            // Contatos deslocados: cargo→nome, email→cargo, celular→email
            ...
            if (plano.includes('+55') || plano.match(/\(\d{2}\)|\d{4,5}-?\d{4}/)) {
                ...
                logger.info(`📱 Celular movido de plano_contratado: "${plano}"`);
                
                // CORREÇÃO 2: PLANO/SLA DESALINHADOS
                // Se celular estava em plano_contratado, plano real está em sla
                const slaOriginal = company.sla || '';
                fixed.plano_contratado = slaOriginal;
                fixed.sla = 'Conforme necessidade'; // SLA padrão
                logger.info(`📋 Plano movido de sla: "${slaOriginal}"`);
            } else {
                fixed.contato_principal_celular = '';
            }
        }
        
        // CORREÇÃO 2.1: PLANO/SLA SEMPRE VERIFICAR (mesmo sem deslocamento de contatos)
        ...
        if (planoAtual.includes('+55') && slaAtual && !slaAtual.includes('+55')) {
            ...
            logger.info(`🔄 Corrigido plano/SLA: "${temp}" ↔ "${fixed.plano_contratado}"`);
        }
        
        // CORREÇÃO 2.2: IMPLANTAÇÃO DESALINHADA
        const implantacao = company.implantacao || '';
        const vencimentoImplantacao = company.vencimento_implantacao || '';
        
        // Se implantação está vazia mas vencimento tem "Não"
        if (!implantacao.trim() && vencimentoImplantacao === 'Não') {
            fixed.implantacao = 'Não';
            fixed.vencimento_implantacao = '';
            logger.info(`📅 Corrigido implantação: vazia → "Não"`);
        }
        
        // CORREÇÃO 3: VALORES BPO INCONSISTENTES
        // Se vl_bpo_contabil tem string "Não" em vez de valor, corrigir
        if (fixed.vl_bpo_contabil === 'Não') {
            fixed.vl_bpo_contabil = '';
            logger.info('💰 Corrigido vl_bpo_contabil de "Não" para vazio');
        }
        
        // CORREÇÃO 4: HONORÁRIO TOTAL VAZIO
        // Se honorario_mensal_total está vazio mas há valores BPO, calcular
        if (!fixed.honorario_mensal_total && fixed.vl_bpo_rh) {
            ...
            logger.info(`💰 Honorário total definido como: ${fixed.vl_bpo_rh}`);
        }
        
        // CORREÇÃO 5: OBSERVAÇÃO CLOSER MAPEAMENTO INCORRETO
        // Se observacao_closer está vazio mas motivo_troca tem conteúdo, mapear corretamente
        if (!fixed.observacao_closer && fixed.motivo_troca) {
            fixed.observacao_closer = fixed.motivo_troca;
            logger.info(`📝 Observação closer mapeada de motivo_troca: ${fixed.motivo_troca.substring(0, 50)}...`);
        }
    ...
    async fetchGroupData(groupName) {
        try {
            // Importa o serviço de busca por grupo diretamente (interno)
            const groupSearchService = require('./group-search.service');
            
            logger.info(`Buscando dados do grupo ${groupName} via serviço interno`);
    ...
    // Função para definir larguras fixas das colunas
    getFixedColumnWidths() {
        logger.info('📐 Aplicando larguras fixas das colunas...');
        ...
        logger.info('📊 Larguras fixas:', fixedWidths.map((w, i) => `${String.fromCharCode(65 + i)}:${w}`).join(', '));
        logger.info('🙈 Colunas J+ serão ocultadas');
        
        return fixedWidths;
    }

    // Função para configurar larguras das colunas fixas e ocultar colunas desnecessárias
    setupColumns(fixedWidths) {
        // Aplica larguras fixas até a coluna I
        const columns = fixedWidths.map(width => ({ width }));
        this.worksheet.columns = columns;
        
        // Oculta todas as colunas após I (coluna 9, índice 8)
        for (let colIndex = 9; colIndex <= 50; colIndex++) { // Oculta J, K, L... até uma coluna alta
        ...
        logger.info('✅ Larguras fixas das colunas configuradas (A-I)');
        logger.info('🙈 Colunas J+ ocultadas');
        logger.info('🔲 Linhas de grade desativadas');
    }

    // Função para adicionar dados à estrutura interna (não escreve na planilha ainda)
    ...
    // Função para adicionar linha vazia à estrutura interna
    addEmptyDataRow() {
        return this.addDataRow(['', '', '', '', '', '', '', '', '']); // A até I (9 colunas)
    }

    // Função para escrever todos os dados na planilha com formatação
    writeDataToWorksheet() {
        logger.info('📝 Escrevendo dados na planilha com formatação...');
        ...
        this.dataRows.forEach((rowData, index) => {
            ...
            if (rowData.data[2]) {
                ...
                const isLabel = this.boldLabels.includes(cellValue) ||
                               ...
                               cellValue.startsWith('Haverá Cobrança') ||
                ...
                if (isLabel) {
                    ...
                    if (isSectionHeader) {
                        for (let colNum = 3; colNum <= 9; colNum++) { // Apenas até coluna I
            ...
            // ================== FORMATAÇÃO DE MOEDA ==================
            // Verifica se é a linha de "Faturamento anual:" para aplicar o formato de moeda
    ...
    // Função para adicionar imagem na linha 3
    async addImageToWorksheet() {
        try {
            // Verifica se o arquivo image.png existe no diretório
            ...
            logger.info('📁 Arquivo image.png encontrado, processando...');

            // Lê o arquivo de imagem
            const imageBuffer = fs.readFileSync(imagePath);
            logger.info(`📊 Tamanho da imagem: ${imageBuffer.length} bytes`);
            ...
            logger.info(`🆔 ID da imagem no workbook: ${imageId}`);

            // Ajusta a altura da linha 3 - menor altura vertical
            const row3 = this.worksheet.getRow(3);
            row3.height = 60; // Altura menor para não ocupar muito espaço vertical
            ...
            logger.info('🖼️  Imagem inserida na linha 3 (C3)');
        } catch (error) {
    ...
    applyMergedCells() {
        this.dataRows.forEach((rowData, index) => {
            if (rowData.data[2] && String(rowData.data[2]).startsWith('Novo cliente -')) {
                ...
                this.worksheet.mergeCells(`C${excelRowNumber}:I${excelRowNumber}`); // Mescla apenas até I
    ...
    addGroupHeader(groupName) {
        ...
        // Linha 3: Será reservada para a imagem (linha vazia aqui, imagem será adicionada depois)
        this.addEmptyDataRow();
        
        // Linha 4: Novo cliente - agora começando na coluna C
                ...
                if (dateStringFromApi && dateStringFromApi.trim()) {
                    ...
                    if (dateStr.includes('/')) {
                        ...
                        if (parts.length === 3) {
                            ...
                            logger.info(`📅 Data DD/MM/YY: "${dateStr}" → ${isoDateString}`);
                        }
                    }
                    // FORMATO 2: YYYY-MM-DD (FORMATO AMERICANO - MM/DD TROCADOS)
                    else if (dateStr.includes('-')) {
                        const parts = dateStr.split('-');
                        if (parts.length === 3) {
                            const year = parts[0];
                            const americanMonth = parts[1]; // Na verdade é o DIA
                            const americanDay = parts[2];   // Na verdade é o MÊS
                            
                            // CORREÇÃO: Tratar como formato americano MM/DD
                            // "2025-01-10" = ano 2025, mês americano 01 (dia real), dia americano 10 (mês real)
                            const realDay = americanMonth;    // 01 → dia 1
                            const realMonth = americanDay;    // 10 → mês 10 (outubro)
                            
                            const isoDateString = `${year}-${realMonth.padStart(2, '0')}-${realDay.padStart(2, '0')}T00:00:00`;
                            dataInicio = new Date(isoDateString);
                            logger.info(`📅 Data YYYY-MM-DD (americano): "${dateStr}" → CORRIGIDO: "${isoDateString}" (dia ${realDay}, mês ${realMonth})`);
                        }
                    }
                    else {
                        dataInicio = new Date(dateStr);
                        logger.info(`📅 Data formato direto: "${dateStr}"`);
                    }
                } else {
                    // Se não houver data, usa a data atual como padrão
                    dataInicio = new Date();
                    logger.info(`📅 Sem data de início, usando atual para ${company.nome_fantasia}`);
...
    }
    ...
    addContractedServices(companies) {
        ...
        // Mapeamento de todos os serviços disponíveis
        const servicosDisponiveis = [
            ...
            { campo: 'bpo_cnd', nome: 'Certidões Negativas' }
        ];

        // Filtra apenas os serviços contratados (que têm "Sim") e numera sequencialmente
        const servicosContratados = [];
        let numeroServico = 1;

        servicosDisponiveis.forEach(servico => {
            if (companies.some(c => c[servico.campo] === 'Sim')) {
                servicosContratados.push(['', '', `Serviço ${numeroServico}:`, servico.nome, '', '', '', '', '']);
                numeroServico++;
                logger.info(`📋 Serviço contratado: ${servico.nome}`);
            }
        });

        // Adiciona os serviços contratados à planilha
        servicosContratados.forEach(servico => {
            this.addDataRow(servico);
        });

        // Se nenhum serviço foi contratado, adiciona uma linha indicando isso
        if (servicosContratados.length === 0) {
            this.addDataRow(['', '', 'Nenhum serviço contratado', '', '', '', '', '', '']);
        }
    ...
    addMainContact(companies) {
        ...
        // Os dados já foram corrigidos na normalização - usar diretamente
    ...
    // Função para adicionar entendimento do negócio - VERSÃO CORRIGIDA
    addBusinessUnderstanding(companies) {
        const chunks = this.chunkArray(companies, 3);
        
        chunks.forEach(chunk => {
            const headerRow = ['', '', 'Entendimento do Negócio'];
            chunk.forEach(company => {
            ...
            chunk.forEach(company => {
                ...
                if (typeof faturamentoString === 'string' && faturamentoString.trim() !== '') {
                    ...
                    // Formato brasileiro: R$ 3.500.000,00 (ponto = milhares, vírgula = decimal)
                    if (valor.includes(',') && valor.lastIndexOf(',') > valor.lastIndexOf('.')) {
                        // Remove pontos (separadores de milhares) e substitui vírgula por ponto
                        valorNumerico = parseFloat(valor.replace(/\./g, '').replace(',', '.'));
                        logger.info(`💰 Formato brasileiro detectado: "${faturamentoString}" → ${valorNumerico}`);
                    }
                    // Formato americano: R$ 3,500,000.00 (vírgula = milhares, ponto = decimal)
                        ...
                        logger.info(`💰 Formato americano detectado: "${faturamentoString}" → ${valorNumerico}`);
                    }
                    // Apenas números com ponto (formato americano simples)
                    else if (valor.includes('.') && !valor.includes(',')) {
                        valorNumerico = parseFloat(valor);
                        logger.info(`💰 Formato americano simples: "${faturamentoString}" → ${valorNumerico}`);
                    }
                    // Apenas números com vírgula (formato brasileiro simples)
                    else if (valor.includes(',') && !valor.includes('.')) {
                        valorNumerico = parseFloat(valor.replace(',', '.'));
                        logger.info(`💰 Formato brasileiro simples: "${faturamentoString}" → ${valorNumerico}`);
                    }
                    // Apenas números inteiros
                    else {
                        valorNumerico = parseFloat(valor);
                        logger.info(`💰 Formato numérico puro: "${faturamentoString}" → ${valorNumerico}`);
                    }

                    if (!isNaN(valorNumerico) && valorNumerico > 0) {
                        // Adiciona o NÚMERO puro. A formatação será feita na escrita da planilha.
                        faturamentoRow.push(valorNumerico);
                    } else {
                        faturamentoRow.push('Sem Faturamento');
                        logger.info(`❌ Erro no faturamento: "${faturamentoString}" → NaN`);
                    }
                } else {
                    faturamentoRow.push('Sem Faturamento');
                }
            ...
            const fields = [
                ...
                { label: 'CT-es de Saída:', field: 'ctes_saida', default: 0, isNumeric: true },
                ...
                { label: 'Funcionários PJ:', field: 'nf_pjs', default: 0, isNumeric: true },
    ...
    addPayroll(companies) {
        ...
        chunks.forEach(chunk => {
            ...
            const fields = [
                { label: 'Funcionários CLT:', field: 'colab_clt', isNumeric: true },
                { label: 'Pró-Labore:', field: 'pro_labore', isNumeric: true },
                { label: 'Estagiários:', field: 'estagiarios', isNumeric: true },
                { label: 'Aprendizes:', field: 'aprendizes', isNumeric: true },
                { label: 'Autônomos - RPA:', field: 'rpa', isNumeric: true },
                { label: 'Domésticas CLT:', field: 'domesticas_clt', isNumeric: true }
            ];
    ...
    async generateForGroup(groupName, companiesData = null) {
        try {
            ...
            logger.info('📊 Coletando todos os dados...');
    ...
    // Método para manter compatibilidade com controller existente
    ...
    fileExists(fileName) {
        try {
            ...
            logger.error('Erro ao verificar existência do arquivo:', error);
            return false;
        }
    }

    // Método para obter o caminho completo de um arquivo
    getFilePath(fileName) {
        return path.join(this.outputDir, fileName);
    }

    // Método para limpeza de arquivos antigos
    cleanupOldFiles(maxAgeInDays = 7) {
        try {
            if (!fs.existsSync(this.outputDir)) {
                logger.info('Diretório de arquivos não existe, nada para limpar');
            ...
            logger.info(`Limpeza concluída: ${deletedCount}/${files.length} arquivos removidos`);
